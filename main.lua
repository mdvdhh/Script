...
do
    local WindUI = getgenv().WindUI or _G.WindUI or nil
    if WindUI and WindUI.CreateWindow then
        local OldCreateWindow = WindUI.CreateWindow
        function WindUI:CreateWindow(Config)
            Config.UiCorner = Config.UiCorner or 10
            local Window = OldCreateWindow(self, Config)
            local WindowBackground = Window.WindowBackground or Window.Frame or Window.Holder or nil
            if WindowBackground and MainFrame:IsA("Instance") then
                local corner = Instance.new("UICorner")
                corner.CornerRadius = UDim.new(0, Config.UiCorner)
                corner.Parent = WindowBackground
            end
            return Window
        end
    end
